...
const updateCompany = [
    ...
    body('gstFile').if((_, { req }) => req.body.company?.gstCheck).custom((_, { req }) => {
        const removed = req.body.removeGSTImg == '1';
        const hasNew = req.files?.gstFile;
        if (removed && !hasNew) throw new Error(req.__('GST Document is required'));
        return true;
    }),

    body('company[panNumber]').notEmpty().withMessage((_, { req }) => req.__('PAN Number is required')).bail().matches(/^[A-Z]{5}[0-9]{4}[A-Z]$/).withMessage((_, { req }) => req.__('Invalid PAN number format')),

    body('panFile').custom((_, { req }) => {
        const removed = req.body.removePanImg == '1';
        const hasNew = req.files?.panFile;
        if (removed && !hasNew) throw new Error(req.__('PAN Document is required'));
        return true;
    }),
    validationHandler
]
